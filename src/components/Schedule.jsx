...
const getSize = () => window.innerWidth >= 1024 ? 'desktop' : window.innerWidth >= 768 ? 'tablet' : 'mobile'
...
export default function Schedule() {
  ...
  const pad = isDesktop ? '40px 40px 24px' : size === 'tablet' ? '32px 24px 24px' : '48px 16px 16px'
  ...
  const toggleExpanded = (templateId) => {
    ...
    else { setExpandedTemplateId(templateId); loadTemplateBlocks(templateId) }
  }
  ...
  const saveTemplate = async () => {
    ...
    if (editingTemplate) await supabase.from('schedule_templates').update({ name: templateForm.name }).eq('id', editingTemplate.id)
    else await supabase.from('schedule_templates').insert({ name: templateForm.name })
    setShowTemplateModal(false); loadAll()
  }

  const deleteTemplate = async () => {
    await supabase.from('schedule_templates').delete().eq('id', editingTemplate.id)
    setExpandedTemplateId(null); setShowTemplateModal(false); loadAll()
  }
  ...
  const saveTplBlock = async () => {
    ...
    if (editingTplBlock) await supabase.from('schedule_template_blocks').update(tplBlockForm).eq('id', editingTplBlock.id)
    else await supabase.from('schedule_template_blocks').insert({ ...tplBlockForm, template_id: tplBlockTemplateId })
    setShowTplBlockModal(false); loadTemplateBlocks(tplBlockTemplateId)
  }
  ...
  const blockRow = (b) => (
    <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderRadius: '20px', background: b.done ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.1)', border: `1px solid ${b.done ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.2)'}`, opacity: b.done ? 0.5 : 1, transition: 'all 0.2s' }}>
      <div onClick={() => toggle(b.id)} style={{ ...iconBg, cursor: 'pointer', flexShrink: 0 }}><TagIcon tag={b.tag} /></div>
  ...
  const templateCard = templates.length > 0 && (
    <div style={{ ...glass, padding: '16px 20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
        {templates.map(t => (
          <button key={t.id} onClick={() => loadTemplate(t.id)} style={{ width: '100%', padding: '10px 14px', borderRadius: '12px', cursor: 'pointer', textAlign: 'left', background: selectedTodayTemplate === t.id ? 'rgba(99,102,241,0.4)' : 'rgba(99,102,241,0.15)', border: `1px solid ${selectedTodayTemplate === t.id ? 'rgba(99,102,241,0.7)' : 'rgba(99,102,241,0.3)'}`, color: '#fff', fontSize: '13px', fontWeight: selectedTodayTemplate === t.id ? '600' : '500', fontFamily: 'Inter, sans-serif' }}>{t.name}</button>
        ))}
      </div>
    </div>
  )
  ...
  const summaryCard = (
    <div style={{ ...glass, padding: '16px 20px' }}>
      ...
      <div style={{ marginTop: '12px', display: 'flex', gap: '8px' }}>
        {[{ label: 'Total', value: blocks.length, color: '#fff' }, { label: 'Done', value: done, color: '#10B981', bg: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)' }, { label: 'Left', value: blocks.length - done, color: '#fff' }].map(s => (
          <div key={s.label} style={{ flex: 1, background: s.bg || 'rgba(255,255,255,0.05)', borderRadius: '10px', padding: '10px', textAlign: 'center', border: s.border || 'none' }}>
  ...
  return (
    <div style={{ padding: pad, display: 'flex', flexDirection: 'column', gap: '20px', boxSizing: 'border-box' }}>

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: isDesktop ? '32px' : '24px', fontWeight: '700', letterSpacing: '-0.5px' }}>Schedule</h1>
      ...
      {view === 'today' && (
        isDesktop ? (
          // Desktop: blocks left, sidebar right
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '20px', alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', minWidth: 0 }}>
              ...
              {blocks.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>{filtered.map(blockRow)}</div>
              )}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {blocks.length === 0 ? (
              <div style={{ ...glass, padding: '40px 32px', textAlign: 'center' }}>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '15px' }}>No blocks for today.</p>
                <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', marginTop: '6px' }}>Load a template above or tap + to add blocks.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>{filtered.map(blockRow)}</div>
            )}
      ...
      {/* ── TEMPLATES ── */}
      {view === 'templates' && (
        <div style={{ display: isDesktop ? 'grid' : 'flex', gridTemplateColumns: isDesktop ? '1fr 1fr' : undefined, flexDirection: isDesktop ? undefined : 'column', gap: '16px', alignItems: 'start' }}>
          {templates.length === 0 ? (
            <div style={{ ...glass, padding: '40px 32px', textAlign: 'center', gridColumn: isDesktop ? '1 / -1' : undefined }}>
      ...
      {/* ── WEEKLY PATTERN ── */}
      {view === 'pattern' && (
        <div style={{ display: isDesktop ? 'grid' : 'block', gridTemplateColumns: isDesktop ? '1fr 1fr' : undefined, gap: '20px', alignItems: 'start' }}>
          <div style={{ ...glass, padding: '20px' }}>
