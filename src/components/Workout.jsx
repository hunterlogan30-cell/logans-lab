...
const STANDARD_EXERCISES = [
  ...
  'Deadlift','Sumo Deadlift','Rack Pull',
  'Plank','Crunch','Cable Crunch','Hanging Leg Raise','Ab Rollout',
  'Barbell Row','Pendlay Row','Meadows Row',
  'Lunges','Bulgarian Split Squat','Step Up','Box Jump',
  'Dumbbell Shoulder Press','Upright Row','Cable Lateral Raise',
]
...
const DotsIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="5" r="1.5" fill={GRAY2}/><circle cx="12" cy="12" r="1.5" fill={GRAY2}/><circle cx="12" cy="19" r="1.5" fill={GRAY2}/></svg>
)
...
function ExerciseDetailSheet({ ex, log, lastLog, onClose, onLogSet, onStartRest }) {
  ...
  const openEdit = (idx, field) => {
    setEditingSet(idx); setEditField(field); setEditValue(sets[idx][field])
  }

  const saveEdit = () => {
    setSets(prev => prev.map((s,i) => i === editingSet ? { ...s, [editField]: editValue } : s))
    setEditingSet(null); setEditField(null)
  }
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 301, background: BG, overflowY: 'auto', animation: 'slideInRight 0.25s ease' }}>
        ...
        <div style={{ padding: '0 20px' }}>
          {sets.map((s, idx) => (
            <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingBottom: '16px' }}>
              ...
              <div onClick={() => openEdit(idx, 'weight')}
                style={{ background: CARD2, borderRadius: '100px', padding: '10px 18px', cursor: 'pointer', minWidth: '80px', textAlign: 'center' }}>
                <span style={{ fontSize: '15px', fontWeight: '600', color: WHITE }}>{s.weight || '—'} lbs</span>
              </div>
              <div onClick={() => openEdit(idx, 'reps')}
                style={{ background: CARD2, borderRadius: '100px', padding: '10px 18px', cursor: 'pointer', minWidth: '72px', textAlign: 'center' }}>
        ...
        <div style={{ height: '1px', background: CARD2, margin: '0 0 20px' }}/>
        ...
        <div style={{ padding: '0 20px calc(60px + env(safe-area-inset-bottom))' }}>
          <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Type anything..."
            rows={6} style={{ width: '100%', background: 'transparent', border: 'none', color: WHITE, fontSize: '16px', fontFamily: 'Inter, sans-serif', outline: 'none', resize: 'none', lineHeight: '1.6', boxSizing: 'border-box' }}/>
        </div>
      </div>

      {editingSet !== null && (
        <>
          <div onClick={saveEdit} style={{ position: 'fixed', inset: 0, zIndex: 400 }}/>
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 401, background: CARD, borderRadius: '24px 24px 0 0', padding: '20px 20px calc(40px + env(safe-area-inset-bottom))' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px' }}>
              <div style={{ width: '36px', height: '4px', borderRadius: '2px', background: GRAY2 }}/>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <button onClick={() => setEditValue(v => String(Math.max(0, parseFloat(v||0) - (editField==='weight'?2.5:1))))}
                style={{ width: '48px', height: '48px', borderRadius: '50%', background: CARD2, border: 'none', color: WHITE, fontSize: '24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
              <input type="number" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus
                style={{ background: 'transparent', border: 'none', color: WHITE, fontSize: '48px', fontWeight: '700', fontFamily: 'Inter, sans-serif', outline: 'none', textAlign: 'center', width: '160px' }}/>
              <button onClick={() => setEditValue(v => String(parseFloat(v||0) + (editField==='weight'?2.5:1)))}
                style={{ width: '48px', height: '48px', borderRadius: '50%', background: CARD2, border: 'none', color: WHITE, fontSize: '24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
            </div>
            <button onClick={saveEdit} style={{ ...PILL_BTN, width: '100%', padding: '16px', fontSize: '16px' }}>Done <ArrowIcon /></button>
          </div>
        </>
      )}
    </>
...
function ExerciseLibrary({ myExercises, onSelect, onClose, programId }) {
  ...
  const combined = [
    ...
    ...STANDARD_EXERCISES.filter(name => !myNames.includes(name.toLowerCase())).map(name => ({ id: name, name, isMine: false }))
  ]
  ...
  const handleAdd = async () => {
    ...
    const { data } = await supabase.from('exercises').insert({
      ...
      weight: newWeight, rest_seconds: parseInt(newRest)||90, program_id: programId
    }).select().single()
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 300, backdropFilter: 'blur(4px)' }}/>
      <div style={{ position: 'fixed', inset: 0, zIndex: 301, background: BG, display: 'flex', flexDirection: 'column', animation: 'slideUp 0.3s cubic-bezier(0.32,0.72,0,1)' }}>
        ...
        <div style={{ padding: '0 20px 16px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: CARD2, borderRadius: '100px', padding: '12px 18px' }}>
            ...
            {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: GRAY, fontSize: '18px' }}>×</button>}
          </div>
        </div>

        {adding && (
          <div style={{ padding: '12px 20px 16px', flexShrink: 0, background: CARD2 }}>
            <p style={{ fontSize: '13px', color: GRAY, marginBottom: '10px' }}>NEW EXERCISE</p>
              ...
              {[['Sets', newSets, setNewSets, 'number'], ['Reps', newReps, setNewReps, 'text'], ['Weight', newWeight, setNewWeight, 'number']].map(([l, v, s, t]) => (
                <div key={l}>
                  ...
                  <input type={t} value={v} onChange={e => s(e.target.value)} placeholder={l}
                    style={{ width: '100%', background: CARD, borderRadius: '10px', border: 'none', padding: '10px 12px', color: WHITE, fontSize: '14px', fontFamily: 'Inter, sans-serif', outline: 'none', boxSizing: 'border-box' }}/>
                </div>
              ))}
            </div>
            <button onClick={handleAdd} style={{ ...PILL_BTN, width: '100%', padding: '14px', marginBottom: '4px' }}>Add exercise <ArrowIcon /></button>
          </div>
        )}

        <div style={{ padding: '16px 20px 8px', flexShrink: 0 }}>
          <h2 style={{ fontSize: '28px', fontWeight: '800', color: WHITE, letterSpacing: '-0.5px' }}>Library</h2>
          <p style={{ fontSize: '13px', color: GRAY, marginTop: '4px' }}>{filtered.length} exercises{search ? ` for "${search}"` : ''}</p>
        </div>

        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px' }}>
          {filtered.map((ex, i) => (
            <div key={ex.id || ex.name} onClick={() => { onSelect(ex); onClose() }}
...
function ActiveWorkoutScreen({ prog, elapsed, running, todayLogs, lastWeekLogs, onToggle, onFinish, onLogSet, onToggleDone, onStartRest }) {
...
function RestTimer({ restSeconds, onDismiss }) {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '90px', left: '50%', transform: 'translateX(-50%)', width: 'calc(100% - 32px)', maxWidth: '390px', zIndex: 400, background: isDone ? '#0d2a1a' : '#1a1a1a', borderRadius: '20px', padding: '16px 20px', boxShadow: '0 24px 48px rgba(0,0,0,0.6)' }}>
      {isDone ? (
...
export default function Workout({ workoutActive, workoutElapsed, workoutRunning, onStartWorkout, onStopWorkout, onToggleTimer, recoveryScore }) {
  ...
  return (
    <div style={{ padding: '56px 20px calc(100px + env(safe-area-inset-bottom))', background: BG, minHeight: '100dvh', boxSizing: 'border-box' }}>
      ...
      {workoutActive && prog && (
        <ActiveWorkoutScreen
          prog={prog} elapsed={workoutElapsed} running={workoutRunning}
          todayLogs={todayLogs} lastWeekLogs={lastWeekLogs}
          onToggle={onToggleTimer} onFinish={handleFinishWorkout}
          onLogSet={handleLogSet} onToggleDone={handleToggleDone}
